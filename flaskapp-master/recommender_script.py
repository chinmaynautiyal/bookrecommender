...
def topten_books(id):
    ...
    toptendf = pd.DataFrame(new_list, columns=['item_id','ISBN', 'bookTitle', 'bookAuthor', 'yearOfPublication', 'publisher', 'imageUrlS', 'imageUrlM', 'imageUrlL','bookID'])
...
def dashboard_df(id):
    ...
    for tuple in df.itertuples():
        if (tuple[1] == id):
            list1 = list(tuple)
            list_master.append(list1)
    
    newdf = pd.DataFrame(list_master, columns= ['index','user_id', 'item_id', 'rating', 'timestamp'])
    mergeddf = pd.merge(newdf, books1682, how='inner', left_on='item_id', right_on = 'bookID')
